...
namespace Pathfinding
{
    ...
    public class PathProcessor
    {
        ...
        {
#if UNITY_2017_3_OR_NEWER
            // Begin profiling for each thread with a unique identifier based on threadID
            ...
            {
#endif
                // Max number of ticks allowed in one run (1 tick = 1/10000 of a millisecond)
                long maxTicks = (long)(10 * 10000);
                // Target tick defines when this run should stop working
                long targetTick = System.DateTime.UtcNow.Ticks + maxTicks;

                while (true)
                {
                    // Pop the next path from the queue to process
                    Path path = queue.Pop();
                    ...
                    // Get the internal interface for path operations
                    IPathInternals ipath = (IPathInternals)path;

                    // Start profiling for this segment of the pathfinding process
                    AstarProfiler.StartFastProfile(0);

                    // Prepare the base path handler (specific to the pathfinding algorithm)
                    ipath.PrepareBase(pathHandler);

                    // Change the path state to "Processing"
                    ipath.AdvanceState(PathState.Processing);

                    // Trigger any callbacks before the path search begins
                    if (OnPathPreSearch != null)
                    {
                        OnPathPreSearch(path);
                    }

                    // Record the starting time for performance profiling
                    long startTicks = System.DateTime.UtcNow.Ticks;

                    // Prepare the path before calculations start
                    ipath.Prepare();

                    AstarProfiler.EndFastProfile(0);

                    // If the path is not yet calculated, we need to start processing it
                    if (path.CompleteState == PathCompleteState.NotCalculated)
                    {
                        // For debugging purposes, store the current path data
                        astar.debugPathData = ipath.PathHandler;
                        astar.debugPathID = path.pathID;

                        // Start profiling for path initialization
                        AstarProfiler.StartFastProfile(1);

                        // Initialize the path, marking it ready for calculation
                        ipath.Initialize();

                        AstarProfiler.EndFastProfile(1);

                        // Continue calculating the path until it is fully calculated
                        while (path.CompleteState == PathCompleteState.NotCalculated)
                        {
                            // Start profiling for each calculation step
                            AstarProfiler.StartFastProfile(2);

                            // Perform part of the pathfinding calculations
                            ipath.CalculateStep(targetTick);

                            AstarProfiler.EndFastProfile(2);

                            // Update the target tick for the next loop iteration
                            targetTick = System.DateTime.UtcNow.Ticks + maxTicks;

                            // If the queue is terminating (e.g., A* is being destroyed), cancel the path
                        ...
                        // Record the total duration of the path calculation for profiling purposes
                        path.duration = (System.DateTime.UtcNow.Ticks - startTicks) * 0.0001F;

#if ProfileAstar
                // Increment counters for completed paths and total search time, if profiling is enabled
                ...
                System.Threading.Interlocked.Add(ref AstarPath.TotalSearchTime, System.DateTime.UtcNow.Ticks - startTicks);
                    ...
                    // Cleanup after the path has been fully processed
                    ipath.Cleanup();

                    // Start profiling for the post-search phase
                    AstarProfiler.StartFastProfile(9);

                    // If there is an immediate callback associated with the path, execute it
                    if (path.immediateCallback != null)
                    {
                        path.immediateCallback(path);
                    }

                    // Trigger any post-search callbacks
                    if (OnPathPostSearch != null)
                    {
                        OnPathPostSearch(path);
                    }

                    // Push the path onto the return queue so the main Unity thread can retrieve it
                    returnQueue.Enqueue(path);

                    // Advance the state to indicate that the path is now in the return queue
                    ipath.AdvanceState(PathState.ReturnQueue);

                    AstarProfiler.EndFastProfile(9);

#if UNITY_2017_3_OR_NEWER
                    // End profiling for this thread's processing of the path
        ...
        if (e is ThreadControlQueue.QueueTerminationException)
#endif
                {
                    // Handle thread termination events gracefully
                ...
                // Terminate pathfinding receivers on unexpected exceptions
                queue.TerminateReceivers();
            }
            finally
            {
#if UNITY_2017_3_OR_NEWER
                // Ensure profiling is correctly ended even in case of errors
                UnityEngine.Profiling.Profiler.EndThreadProfiling();
#endif
            }
#endif

            // Log an error if the code unexpectedly reaches here (this should never happen)
