...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    {
#if UNITY_WEBGL
    return 0;  // WebGL doesn't support threads
#else
        if (count == ThreadCount.AutomaticLowLoad || count == ThreadCount.AutomaticHighLoad)
        {
            ...
            if (memory <= 0)
            {
                Debug.LogError("Machine reporting <= 0 bytes of RAM. Assuming 1 GiB.");
                memory = 1024; // Assume at least 1 GiB of RAM
            }

            // Adjust to enable multithreading even for systems with more modest resources
            if (logicalCores > 1 && memory > 512)
            {
                return Mathf.Min(logicalCores, 8); // Cap the thread count to avoid oversubscription
            }

            return 1; // Default to 1 thread if resources are low
        }
        else
        {
            return Mathf.Max(1, (int)count); // Allow user-defined count
        }
#endif
    }
